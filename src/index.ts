...
import { getSafeInfo, getPendingTransactions, postTransactions } from './api';
...
class Safe {
  ...
  constructor(safeAddress: string, version: string, provider: providers.Web3Provider, network = "1") {
  ...
  getThreshold() {
    const threshold = this.contract.getThreshold()
  ...
  async postTransaction(transaction: SafeTransaction, hash: string) {
    ...
    await postTransactions(this.safeAddress, {
      safe: safeAddress,
  ...
  async getBalance(): Promise<BigNumber> {
    return this.contract.getBalance(this.safeAddress)
  }
